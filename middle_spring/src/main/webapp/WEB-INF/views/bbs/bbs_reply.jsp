...
<body>
   <div id="bsW_wrap">
      ...
      <form method="post" action="bbs_reply_ok.do" onsubmit="return write_check();" >
      <%--답변글 히든값 --%>
      <input type="hidden" name="bbs_ref" value="${b.bbs_ref}" > <%--원본글과 답변글을 묶어주는 글 그룹
      번호 역할을 한다. --%>
      <input type="hidden" name="bbs_step" value="${b.bbs_step}" > <%--원본글과 답변글을 구분하는 번호값이
      면서 몇번째 답변글인가를 알려준다. --%>
      <input type="hidden" name="bbs_level" value="${b.bbs_level}" ><%--답변글 정렬순서 --%>
      
      <%--페이징에서 책갈피 기능 히든값 -> 쪽번호 --%>
      <input type="hidden" name="page" value="${page}" >
         ...
         <div id="bsW_menu">
            <input type="submit" value="답변" /> <input type="reset" value="취소"
               onclick="location='bbs_cont.do?bbs_no=${b.bbs_no}&page=${page}&state=cont';">
                <input type="button"
