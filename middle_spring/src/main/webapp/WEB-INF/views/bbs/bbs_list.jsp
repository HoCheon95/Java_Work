...
<body>
	<form method="get" action="bbs_list.do">
		<div id="bList_wrap">
					...
					<c:forEach var="b" items="${blist}">
						<tr>
							<td align="center"><c:if test="${b.bbs_step ==0}">
									<%--원본글일때만 실행 --%>
       ${b.bbs_ref} <%-- 원본글과 답변글을 묶어주는 글그룹번호 출력->원본글 그룹번호만 출력되면 된다. 답변글 그룹번호도 같기때문에 출력할 필요가 없다. --%>
      </c:if></td>
      ...
      &nbsp;<%--답변글 들여쓰기 --%>
							...
							<td align="center">							
							 <c:if test="${b.bbs_attached_file == 7}"> <%-- 첨부파일이 있는 경무만 첨부 이미지 출력 --%>
							 <img src="./images/bbs/disk.png" alt="디스크 이미지" />
							 </c:if>
							</td>
						</tr>
					</c:forEach>
   ...
   [이전]&nbsp;
					...
					<c:if test="${page >1}">
						<a
							href="bbs_list.do?page=${page-1}&find_field=${find_field}&find_name=${find_name}">[이전]</a>&nbsp;
   </c:if>
					...
					<c:forEach var="a" begin="${startpage}" end="${endpage}" step="1">
						...
						<c:if test="${a != page}"> <%--현재 쪽번호가 선택 안된 경우 --%>
							<a
								href="bbs_list.do?page=${a}&find_field=${find_field}&find_name=${find_name}">[${a}]</a>&nbsp;
    </c:if>
					</c:forEach>
					...
					<c:if test="${page<maxpage}">
						<a
							href="bbs_list.do?page=${page+1}&find_field=${find_field}&find_name=${find_name}">[다음]</a>
					</c:if>
				</c:if>
			...
			<div id="bList_menu">
				<input type="button" value="글쓰기"
					...
					<%-- ?page=쪽번호를 get방식으로 전달하는 이유는 페이징 목록에서 내가 본 쪽번호로 바로 이동하기 위한 책갈피 기능을 구현하기 위해서이다. --%>
				<c:if test="${(!empty find_field) && (!empty find_name)}">
