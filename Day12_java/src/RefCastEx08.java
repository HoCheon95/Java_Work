...
public class RefCastEx08 {
	...
	public static void main(String[] args) {
		...
		System.out.println("m.x = " + m.x); // 100 -> 부모와 자손에서 동일한 멤버변수명이 정의된 경우에는
		                                    // 호출되는 멤버변수 결정은 참조변수 m 타입에 의해서 결정된다. 여기에서 부모타입인
		                                    // Mother08이기 때문에 부모에서 정의된 동일 멤버변수명이 호출된다.
		m.m08();// 업캐스팅 이후 오버라이딩 한 메서드 호출은 참조변수 m이 실제 가리키는 객체타입에 의해서 결정 된다.
		        // 여기서는 자손 Child08이기 때문에 오버라이딩 한 메서드를 호출한다.
